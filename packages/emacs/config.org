  ...
  (defun my-update-mode-line ()
    (dolist (window (window-list))
      (let* ((window-below
             ...
             (buffer-below
              (and window-below
                   (window-buffer window-below)))
             (header-line-below
              (and buffer-below
                   (buffer-local-value 'header-line-format buffer-below))))
        (set-window-parameter window 'mode-line-format
                              (cond ((minibufferp buffer-below) (list " "))
                                    (header-line-below nil)
                                    (t (list " ")))))))

  (add-hook 'window-configuration-change-hook #'my-update-mode-line)
#+end_src
...
#+begin_src emacs-lisp :tangle no
  (setopt mode-line-format
          '("%e"
            ...
            (:eval (when skk-mode
                     (cond (skk-latin-mode "ASCII  ")
                           (skk-jisx0208-latin-mode "全英  ")
                           (skk-katakana "カナ  ")
                           (t "かな  "))))
 ...
 #+begin_src emacs-lisp
   ...
   (defun modus-themes-set-custom-faces (&rest _)
     (modus-themes-with-colors
       (custom-set-faces
        '(region ((t :extend nil)))
        `(mode-line ((,c :height 0.1
                         :foreground ,border
                         :background ,bg-main
                         :overline t
                         :box nil)))
        `(mode-line-inactive ((,c :height 0.1
                                  :foreground ,border
                                  :background ,bg-main
                                  :overline t
                                  :box nil)))
...
#+begin_src emacs-lisp
  (use-package tempel
    ...
    :config
    (setopt tempel-path "@templates@/*.eld")
...
#+begin_src emacs-lisp
  (use-package transient
    :config
    (setopt transient-mode-line-format '(" ")))
#+end_src
...
#+begin_src emacs-lisp
  (use-package vundo
    :bind ("C-x u" . vundo)
    :preface
    (defun vundo-hide-header-line ()
      (setq header-line-format nil))
    :init
    (add-hook 'vundo-mode-hook #'vundo-hide-header-line))
#+end_src
